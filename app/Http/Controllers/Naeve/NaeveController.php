...
class NaeveController extends Controller
{
    ...
    public static function accuracy()
    {

        $studentcourses = StudentCourses::query()->select('performance','attendance','pass/fail')
            ->where('course_id','=','IS215')
            ->get();
        $samples  = TrainingSet::query()->select('performance', 'attendance','fail/pass')
            ->get();
        //return $samples;
        $accure=0;
        foreach ($studentcourses as $stud)
        {
            $yes='False';
            ...
            foreach ($samples as $sample)
            {
                ...
                if($stud->performance==$sample->performance && $stud->attendance==$sample->attendance &&$stud-> pass==$sample->fail)
                {
                    $yes='True';
        ...
        return $accure/count($studentcourses)*100;
