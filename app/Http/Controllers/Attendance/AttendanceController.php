...
class AttendanceController extends Controller
{
    ...
    public static function addAttendence($QrContent,$studentID){
        $result = Session::query()->where('session_id','=',$QrContent->sessionID)->first();
        $available = $result['isAvailable'];
        if ($available == 1){
            if(Attendance::where('session_id',$QrContent->sessionID)
            ...
            return json_encode('error, Session ended');
        }
    ...
    */
    ...
    public function attendLecture(Request $request){
        ...
        return $this->addAttendence($newRequest,$request->studentID);
